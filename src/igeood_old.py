...
from src.estimators_igeood import (hidden_feature_estimator,
                            run_logits_centroid_estimator)
from src.measures_igeood import *
...
def main(
    ...
    count = min(len(combine_in_score), len(combine_out_score))
    combine_in_score = combine_in_score[:count]
    combine_out_score = combine_out_score[:count]
...
def igeoodwb_score(
    ...
    if cov_mat_ood is not None:
        if cov_mat_ood == "ADV":
            ...
            if nn_name =='resnet_ustc_task_0_in':
                cap = 10
            elif nn_name == 'resnet_ustc_task_1_in':
                cap = 10
            elif nn_name == 'resnet_ustc_task_2_in':
                cap = 20
            else:
                cap = 2000
            print(f'模型名称: {nn_name}')
        ...
        with torch.no_grad():
            for layer_idx, out_feature in enumerate(out_features):
                ...
                if cov_mat_out is not None:
                    ...
                    score2= torch.mean(score2, dim=1)
